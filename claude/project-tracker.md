...
## 15. v0.8.1 — screens locked to the device (14 Sep 2026)

**Report:** on the phones/PDAs the app is used on, a sideways finger swipe moved the whole page off the screen. In emulation no page overflowed, so the causes were device-side: iOS zooms the page when a text field under 16px gets focus (the login form, the concierge compose, the back-office fields were 14–15px), and after that a wider-than-screen page pans sideways; older PDA WebViews do not know `overflow-x: clip`, which was the only sideways guard; and two back-office screens really did overflow at 360px (partner statement dates, WhatsApp URL/Send button).

**Fix (all screens):** viewport `maximum-scale=1, user-scalable=no` (no pinch, double-tap or focus zoom — the page is fixed to the device); `html{overflow-x:hidden; overflow-x:clip}` (hidden for old WebViews, clip where supported so sticky bars keep working) and `body{touch-action:pan-y; overscroll-behavior-x:none}` — a sideways pan is refused at the page level while chips, stations, the back-office bottom bar, tables (`.tbl-wrap`) and segmented controls remain their own horizontal scroll containers; every text control is 16px on touch devices at every specificity; `input,select{min-width:0}`, `img/svg/video/canvas{max-width:100%}`, `code{overflow-wrap:anywhere}`; back-office header rows and toolbars wrap on PDA widths, the WhatsApp page and board pack use the collapsing `grid2/grid3` layouts, board tables scroll in wrappers, `vh` fallbacks before every `dvh`. **Verified:** Playwright audit of 56 routes (buyer, organiser, back office) at 320 and 360px in EN and AR — zero elements outside the viewport, `scrollWidth == clientWidth` everywhere; simulated touch swipes: chips rail 160px, bottom bar 399px and a table 228px pan inside themselves, page `scrollX` stays 0 even with a 1200px element injected, vertical swipe scrolls, tabs stay pinned. If a device still slides the page sideways after this, it is the browser's own edge-swipe back/forward gesture (system-level; opening the app from the home-screen icon avoids it).
