...
| Venue tools (dashboard with forecasts, nudges and copilot; poster-to-listing with several offers; full listing editor with pricing assistant and add-ons; marketing kit — captions, broadcast, print-ready poster and QR stand; Promote packages; plans Free/Pro/Venue; promoters with tiers and share kits; promo codes; insights and buyer export; refunds; developers — API keys and webhooks; door scanner that works offline; partner finance) | `/org`, `/org/new`, `/org/e/[id]`, `/org/kit/[id]`, `/org/promote/[id]`, `/org/plan`, `/org/promoters`, `/org/codes`, `/org/insights`, `/org/refunds`, `/org/developers`, `/org/door`, `/org/finance` | same deployment |
| Back office (orders, double-entry ledger, partners, settlements with statement drafts, invoices, provider reconciliation, revenue-share rules, reports with a 30-day forecast, streams, promotions queue, moments review, team and plans, **settings — theme, tabs, features, brand**, tenants, **fraud & door**, **WhatsApp concierge conversations**, monthly **board pack**, client errors, audit, ask-the-ledger) | `/admin/*` | Vercel project **whatsup-backoffice-app** (https://whatsup-backoffice-app.vercel.app, root URL redirects to `/admin`) |
...
What the client app shows is configured per tenant in the back office (Settings): one of four themes (Cedar, Volt, Sunset Beirut, Ultraviolet), which bottom tabs exist, thirty feature switches for pages, sheets and modals, and the brand words — so a rebranded copy of the app on another server is a settings change plus icon files (`docs/DESIGN.md`, "White label / rebrand checklist").
...
supabase/   migrations (18), edge functions (create-order, pay, payment-webhook, scan, wallet, notify, wa-otp-hook, wa-inbound), demo seeds, sync-lib.sh
claude/     project tracker (working log for the AI-assisted build)
docs/       architecture, deployment, design, upgrade strategy, API, WhatsApp templates, founding venues, second-country runbook
...
* One Listing (`events` row, `kind` event | venue | stay | pass) sells Offers: tiers of a `kind` (ticket, daypass, item, stay, pass), VIP tables and deals. Buyer fees by type, shown all-in: tickets **5% + $0.50**, day passes and items **5%**, stays **4%**, tables / passes / deals **none**. Organiser receives **face − organiser fee (3% Free · 2.5% Pro · 2% Venue) − ~2.5% card processing + table deposits**; free things carry no fees. A valid pass covers `member_free` offers at the venues it lists.
* **Access first (v0.9).** Every offer has a role: **entries** (tickets, day passes, stays, memberships, entry-inclusive tables) let people in — `admits` per unit, a cabana admits six — and **services** (items, add-ons, table packages) are for people inside: they need an entrance in the same order or one the buyer already holds for that listing (`held_access()`), per-person services are capped at the headcount, the door scans a service QR as a **pickup** only after the holder's entrance was scanned, and checked-in counts people, never pickups. An organiser can mark a service "no entry needed" (a shuttle); a listing cannot be published with services and no entry offer. Rule and rationale in `claude/access-first-rule.md` (project) — the concierge and the WhatsApp bot quote entries and bundle the entrance with any service they book.
...
* Checkout runs in the `create-order` edge function: holds inventory (or fulfils from the resale pool when a tier is sold out), prices the order from the tenant's fee snapshot with add-ons (refund protection; listing extras such as fast lane and parking, priced server-side from `events.addon_options`) and fan credit, and either settles instantly (sandbox), hands the buyer to a hosted checkout (`pay` → Stripe / Areeba / Whish, confirmed by `payment-webhook`) or creates a reservation (OMT, cash at door) that expires before doors. Tickets carry a static HMAC token (`WU1.<id>.<sig>`) and a per-ticket key from which the wallet mints a rotating token every two minutes (`WU2.<id>.<slot>.<sig>`).
* WhatsApp is also a sales channel (v0.8): the `wa-inbound` function answers people who message the business number from the live catalogue and hands them a checkout link with the offer preset; the back office sees every conversation and can take over.
