  ...
  return (
    <>
      <Band h={120} top={t("wm1")} main={t("country")}>
        <CityPill city={city} cities={cities} label={cityLabel} />
...
function RefCapture({ code }: { code: string }) {
  ...
  return <script dangerouslySetInnerHTML={{ __html: `try{localStorage.setItem('wu-ref',${JSON.stringify(c)});fetch('${SUPABASE_URL}/rest/v1/rpc/promoter_click',{method:'POST',headers:{'Content-Type':'application/json','apikey':'${SUPABASE_ANON_KEY}','Authorization':'Bearer ${SUPABASE_ANON_KEY}'},body:JSON.stringify({p_tenant:'${TENANT}',p_code:${JSON.stringify(c)}})}).catch(function(){})}catch(e){}` }} />;
}
