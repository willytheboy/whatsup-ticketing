      ...
      {!d ? <Empty>Loading…</Empty> : (
        <div style={{ maxWidth: 900 }}>
          ...
          <section className="panel" style={{ marginTop: 12 }}><h2>Top listings</h2>
            {!d.top.length ? <Empty>No paid orders this month.</Empty> : <table className="tbl"><thead><tr><th>Listing</th><th className="r">Orders</th><th className="r">Gross</th></tr></thead><tbody>{d.top.map((e: any, i: number) => <tr key={i}><td>{e.title}</td><td className="r">{e.orders}</td><td className="r">{money(e.gross)}</td></tr>)}</tbody></table>}
          </section>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
            <section className="panel"><h2>Payment methods</h2>{!d.methods.length ? <Empty>—</Empty> : <table className="tbl"><tbody>{d.methods.map(([m, v]: [string, number]) => <tr key={m}><td>{m}</td><td className="r">{money(v)}</td></tr>)}</tbody></table>}</section>
            <section className="panel"><h2>Partners settled</h2>{!d.settlements.length ? <Empty>No settlements closed this month.</Empty> : <table className="tbl"><tbody>{d.settlements.slice(0, 12).map((s: any, i: number) => <tr key={i}><td>{s.partners?.name}<div className="dim">{s.partners?.kind} · {s.status}</div></td><td className="r">{signed(Number(s.amount))}</td></tr>)}</tbody></table>}</section>
            <section className="panel"><h2>Promotions sold</h2>{!d.promos.length ? <Empty>None this month.</Empty> : <table className="tbl"><tbody>{Object.entries(d.promos.reduce((a: Record<string, [number, number]>, p: any) => { const k = p.package ?? "?"; a[k] = [(a[k]?.[0] ?? 0) + 1, (a[k]?.[1] ?? 0) + Number(p.price)]; return a; }, {})).map(([k, [n, amt]]: any) => <tr key={k}><td>{k} × {n}</td><td className="r">{money(amt)}</td></tr>)}</tbody></table>}</section>
            <section className="panel"><h2>WhatsApp concierge</h2><table className="tbl"><tbody><tr><td>Conversations</td><td className="r">{d.wa.conversations}</td></tr><tr><td>Messages</td><td className="r">{d.wa.messages}</td></tr><tr><td>Checkout links handed out</td><td className="r">{d.wa.carts}</td></tr><tr><td>Handed to a person</td><td className="r">{d.wa.handoffs}</td></tr></tbody></table></section>
          </div>
